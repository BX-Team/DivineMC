...
if [ "$number" -gt 0 ]; then
  while IFS= read -r line; do
    ...
    commit_summary=$(echo "$line" | cut -d' ' -f2-)
    commit_time=$(git show -s --format=%cI "$commit_sha")
    commits_json=$(echo "$commits_json" | jq --arg sha "$commit_sha" --arg summary "$commit_summary" --arg at "$commit_time" \
      '. + [{"sha": $sha, "summary": $summary, "at": $at}]')
