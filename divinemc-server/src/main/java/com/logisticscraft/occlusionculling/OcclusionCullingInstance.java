...
public class OcclusionCullingInstance {
    ...
    public boolean isAABBVisible(Vec3d aabbMin, Vec3d aabbMax, Vec3d viewerPosition) {
        try {
            ...
            // Failsafe: the server port reads chunk data concurrently, so a torn read must
            // never break culling - treat the entity as visible and move on.
            return true;
        }
    }
    ...
    private boolean isVisible(Vec3d start, Vec3d[] targets, int size) {
        ...
        for (int v = 0; v < size; v++) {
            ...
            if (dimensionX == 0f) {
                ...
                x_inc = 1; // target point is horizontally greater than starting point so increment every step by 1
                intersectCount += MathUtilities.floor(target.x) - x; // increment total amount of intersecting cells
                // calculate the next horizontal intersection point based on the position inside the first cell
                t_next_x = (float) ((x + 1 - start.x) * dimFracX);
            } else {
                x_inc = -1; // target point is horizontally smaller than starting point so reduce every step by 1
            ...
            if (dimensionY == 0f) {
                ...
                y_inc = 1; // target point is vertically greater than starting point so increment every step by 1
                intersectCount += MathUtilities.floor(target.y) - y; // increment total amount of intersecting cells
                // calculate the next vertical intersection point based on the position inside the first cell
                t_next_y = (float) ((y + 1 - start.y) * dimFracY);
            } else {
                y_inc = -1; // target point is vertically smaller than starting point so reduce every step by 1
            ...
            if (dimensionZ == 0f) {
                z_inc = 0;
                t_next_z = dimFracZ; // don't increment because the Vec3d is perfectly straight on this axis
            } else if (target.z > start.z) {
                z_inc = 1; // target point is greater than starting point so increment every step by 1
                intersectCount += MathUtilities.floor(target.z) - z; // increment total amount of intersecting cells
                // calculate the next intersection point based on the position inside the first cell
                t_next_z = (float) ((z + 1 - start.z) * dimFracZ);
            } else {
                z_inc = -1; // target point is smaller than starting point so reduce every step by 1
    ...
    private boolean stepRay(Vec3d start, int currentX, int currentY,
        ...
        for (; n > 1; n--) { // n-1 times because we don't want to check the last block
            ...
            if (t_next_y < t_next_x && t_next_y < t_next_z) {
                // next cell is upwards/downwards because the distance to the next vertical
